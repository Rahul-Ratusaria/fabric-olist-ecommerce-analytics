...
Clean, type, validate, enrich, deduplicate, and quarantine Bronze
records to produce trusted analytical entities for Gold modeling.
...
These are row-level enrichments. Business aggregations and dashboard
KPIs remain part of the Gold and semantic-model layers.
...
![Core Silver
tables](../screenshots/06-silver-layer/01-core-silver-tables.png)

![Orders
schema](../screenshots/06-silver-layer/02-silver-orders-schema.png)

![Order
features](../screenshots/06-silver-layer/03-order-derived-features.png)

![Quarantine
summary](../screenshots/06-silver-layer/04-core-quarantine-summary.png)
...
Seller identifiers and geographical attributes are standardized at one
row per seller.

Critical invalid identifiers and ZIP prefixes are routed to
`quarantine_sellers`.
...
The Silver layer provides cleaned and validated analytical entities,
supporting reference tables, quarantine outputs, derived operational
features, and reconciliation/audit evidence for downstream Gold
modeling.
